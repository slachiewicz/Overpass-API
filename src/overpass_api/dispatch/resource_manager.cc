...
void Resource_Manager::health_check(const Statement& stmt, uint32 extra_time, uint64 extra_space)
{
  ...
  if (elapsed_time > max_allowed_time)
  {
    ...
    Logger logger(transaction->get_db_dir());
    ostringstream out;
    out<<"Timeout: runtime "<<elapsed_time<<" seconds, size "<<size<<" bytes, "
        "in line "<<stmt.get_line_number()<<", statement "<<stmt.get_name();
    logger.annotated_log(out.str());
    
    throw Resource_Error(true, stmt.get_name(), stmt.get_line_number(), size, elapsed_time);
  }
  ...
  if (size > max_allowed_space)
  {
    ...
    Logger logger(transaction->get_db_dir());
    ostringstream out;
    out<<"Oversized: runtime "<<elapsed_time<<" seconds, size "<<size<<" bytes, "
        "in line "<<stmt.get_line_number()<<", statement "<<stmt.get_name();
    logger.annotated_log(out.str());
    
    throw Resource_Error(false, stmt.get_name(), stmt.get_line_number(), size, elapsed_time);
  }  
}
