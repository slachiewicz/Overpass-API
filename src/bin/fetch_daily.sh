...
TMP_DIFF=/tmp/day/diff.osc.gz
TMP_DIFF_UNCOMPRESS=/tmp/day/diff.osc
TMP_STATE=/tmp/day/state.txt
DIFF_URL=$1
EXEC_DIR="`dirname $0`/"
...
fetch_and_apply_minute_diff()
{
  ...
  cat $TMP_DIFF_UNCOMPRESS | $EXEC_DIR/update_database --flush-size=0 $2 >> log 2>&1
  ret=$?
  rm $TMP_DIFF 2>/dev/null
  rm $TMP_DIFF_UNCOMPRESS 2>/dev/null
...
do
{
  ...
  if test -f "stop_processing"; then 
  {
